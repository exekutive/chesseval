...
# font manager is very noisy
logging.getLogger('matplotlib.font_manager').setLevel(logging.WARNING)

# Custom Locator to enforce a minimum tick interval
class ChessturnMajLocator(ticker.Locator):
    ...
    def __call__(self):
        ...
        ticks = list(ticker.MaxNLocator(integer=True).tick_values(vmin, vmax))
        if len(ticks) > 1 and ticks[1] - ticks[0] < self.min_interval:
            ticks = list(ticker.MultipleLocator(1.0).tick_values(vmin, vmax))
...
def draw_eval(game_eval):
    ...
    with plt.rc_context(cfg.rcstyle):
        ...
        fig1.suptitle( "Stockfish Evaluation",
                      fontweight    = 'bold',
                      fontsize      = 18)
        ...
        ax_prob.spines['bottom'].set_position(('outward', 1))
        ...
        if cfg.cl_args['noadv'] is False:
            ...
            # make graph limits symmetrical around y=0
            ylim = max(abs(val) for val in ax_adv.get_ylim())
